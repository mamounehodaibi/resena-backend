...
async function seedMenu() {
  ...
  await insertItem(c1, 'Ensalada de Burrata', 'Con tomate ecologico, salsa de mango, albahaca', '', 8.90, null, 1);
  await insertItem(c1, 'Ensalada de Ventresca', 'Tomate ecologico, ventresca, aguacate', '', 13.50, null, 2);
  ...
  await insertItem(c1, 'Tabla La Resena', 'Jamon iberico, queso curado, salmorejo', '', 14.50, null, 5);
  await insertItem(c1, 'Ensaladilla de Gamba', '', '', null, JSON.stringify({Entera:12.00,Media:7.50,Tapa:4.50}), 6);
  await insertItem(c1, 'Ensaladilla de Salmon Ahumado', 'Salmon ahumado, alcaparras, maiz, pepinillo', '', null, JSON.stringify({Entera:13.50,Media:8.50,Tapa:5.50}), 7);
  ...
  await insertItem(c2, 'Couscous de Pollo', 'Con semola, verdura, cebolla caramelizada', 'Reserva con un dia de antelacion - minimo 2 personas', 14.50, null, 6);
  await insertItem(c2, 'Couscous de Ternera', 'Con semola, verdura, cebolla caramelizada', 'Reserva con un dia de antelacion - minimo 2 personas', 16.50, null, 7);

  const c3 = (await insertCat('empezamos', 'Empezamos', 3)).rows[0].id;
  await insertItem(c3, 'Tortilla Trufada', '', '', 9.50, null, 1);
  await insertItem(c3, 'Flamenquin La Resena', 'Con 4 quesos - especialidad de la casa', '', 13.40, null, 2);
  await insertItem(c3, 'Flamenquin Cordoces', '', '', 10.90, null, 3);
  await insertItem(c3, 'Hamburguesa de Ternera', 'Con bacon y cebolla caramelizada, salsa casera', '', 12.00, null, 4);
  await insertItem(c3, 'Revuelto de Patatas', 'Con patata, jamon y huevos', '', 12.00, null, 5);
  ...
  await insertItem(c3, 'Berenjenas Crujiente', 'Con miel de cana y queso de cabra', '', null, JSON.stringify({Entera:10.00,Media:7.00}), 10);
  ...
  await insertItem(c6, 'Tiramisu', '', '', 5.50, null, 3);
  await insertItem(c6, 'Coulant de Chocolate', 'Con helado de vainilla', '', 5.50, null, 4);
  await insertItem(c6, 'Te Marroqui para 2 Personas', 'La experiencia perfecta para compartir', '', 4.00, null, 5);
